...
# Harmonic sinusoids
def harmonics(params=None, afun=lambda x: pylab.exp(-0.5*x), pfun=lambda x: pylab.rand()*2*pylab.pi):
    """
    ::
...
def shepard(params=None, num_octaves=7, center_freq=440, band_width=150):
    """
    ::
...
def devils_staircase(params=None, num_octaves=7, num_steps=12, step_size=1, hop=4096, 
    ...
    for i in pylab.arange(num_steps):
        ...
        params['f0'] = freq
        s = shepard(params, num_octaves=num_octaves, center_freq=center_freq, band_width=band_width)
        s[0:wlen] *= h[0:wlen]
...
def _check_noise_params(params):
    ...
    params['sr'] = float(params.get('sr',p['sr']))
    params['num_points'] = int(params.get('num_points',p['num_points']))
    params['num_harmonics'] = int(params.get('num_harmobics',p['num_harmonics']))
    return params

# Combine harmonic sinusoids and noise signals
def noise(params=None, noise_fun=pylab.rand):
    """
    ::
...
def balance_signal(sig, balance_type="maxabs"):
    ...
    if balance_type==balance_types[4]:
        return sig    
    raise TestSignalError("signal balancing type not supported:", balance_type)
