...
class WavOpen:
    ...
    def next(self):
        if self.index < self.num_frames - self.n + 1:
            rawdata = self.sound.readframes(self.n)
            if rawdata:
                signal = wave.struct.unpack('%ih' %self.n*self.num_channels, rawdata) # transform to signal
                del rawdata
...
def _wav_write(signal, wav_name, sample_rate):
    ...
    if HAVE_AUDIOLAB:
        ...
        for i in range(signal.shape[1]):
            b_signal += wave.struct.pack('h',32767*signal[0,i]) # transform to C-style binary string
            if signal.shape[0]>1:
                b_signal += wave.struct.pack('h',32767*signal[1,i]) # transform to C-style binary string            
