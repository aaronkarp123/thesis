...
class adb:
    ...
    @staticmethod
    def cov_vector(data, mn=True, cv=True, num_blocks=1, deltas=True, num_frames=None, norm_type=0, post_norm=0):
        """
        ::
        ...
        if norm_type==1:
            data /= pylab.np.atleast_2d(data.sum(1)).T
        if norm_type==2:
            data /= pylab.np.atleast_2d(pylab.np.sqrt((data**2).sum(1))).T
        if norm_type==3:
            data /= pylab.np.atleast_2d(data.max(1)).T
        ...
        if post_norm:
            if norm_type==1:
                features /= pylab.np.atleast_2d(features.sum(1)).T
            if norm_type==2:
                features /= pylab.np.atleast_2d(pylab.np.sqrt((features**2).sum(1))).T
            if norm_type==3:
                features /= pylab.np.atleast_2d(features.max(1)).T
        return features            
